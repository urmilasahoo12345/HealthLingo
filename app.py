...
# ================================
def get_multilingual_reply(answer_en, user_input):
    try:
        user_lang = detect(user_input)
    except:
        user_lang = "en"

    # If input is English, keep English
    if user_lang == "en":
        return answer_en, "en"

    # Otherwise, translate to user’s language
    try:
...
bot_reply_text = None  # store latest bot reply
...
if user_input:
    ...
    # 4. Translate based on input language
    final_reply, lang_used = get_multilingual_reply(answer_en, user_input)

    # Save for TTS
    bot_reply_text = final_reply
    st.session_state.messages.append({"role": "bot", "content": f"🤖 {final_reply}"})
